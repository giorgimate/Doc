...
namespace DocAppointmentAPI.JwtFeatures
{
    public class JwtHandler
    {
        ...
        public bool CheckSelfOrAdmin(HttpRequest request, string userId)
        {
            var jwtToken = GetToken(request);
            var jwtUserId = GetData(jwtToken, ClaimTypes.NameIdentifier.ToString());
            var jwtUserRole = GetData(jwtToken, ClaimTypes.Role.ToString());

            return jwtUserId == userId || jwtUserRole == "Admin";
        }
    }
}
